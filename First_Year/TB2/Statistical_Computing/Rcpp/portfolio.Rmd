...
double mse_lambda(const double lambda, const NumericVector x,const NumericVector y,const NumericVector x_new, const NumericVector y_new){
...
NumericVector lambda_cv_Rcpp(const NumericVector x, const NumericVector y, const NumericVector groups) {
    ...
    class Mse : public Functor {
      ...
      double operator()(const arma::vec& log_lambda) override {
        ...
        // std::function<double(const double log_lambda)> mse = { mse_lambda(log_lambda, x_train, y_train, x_test, y_test)};
...
```{r}
muSmoothAdapt <- mean_var_KRS( y, x, xnew, 0.06)
...
NumericVector mean_var_KRS_Rcpp(const NumericVector y, const NumericVector x, const NumericVector xnew, const double lambda) {
  ...
  w = w / mean(w);
  for(int ii = 0; ii < nnew; ii++){
    out[ii] = sum(dnorm(x, xnew[ii], lambda *w[ii]) * y)/ sum( dnorm(x, xnew[ii], lambda *w[ii]));
  }
...
We now compute the Rcpp version and check it is consistent with our previous results:
```{r}
muSmoothAdaptRcpp <- mean_var_KRS_Rcpp( y, x, xnew, 0.06)
max(abs(muSmoothAdapt - muSmoothAdaptRcpp))
```

And we can now compare the running time from these:
```{r}
microbenchmark("R" = mean_var_KRS( y, x, xnew, 0.06),
               "Rcpp" = mean_var_KRS_Rcpp( y, x, xnew, 0.06))
```
