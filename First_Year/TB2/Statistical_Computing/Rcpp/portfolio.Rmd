...
// [[Rcpp::export]]
double mse_Rcpp(const double log_lambda, const NumericVector x, const NumericVector y, const NumericVector x_new, const NumericVector y_new){
  double lambda = exp(log_lambda);
  
  NumericVector fitted = meanKRS_Rcpp_I(x, y, x_new, lambda);
  
  int value = sum(pow(fitted - y_new, 2));
  return value;
}


// [[Rcpp::export]]
double lambda_cv_Rcpp(const NumericVector x, const NumericVector y, const double nfolds = 5){
  int n = x.size();
  
  NumericVector groups = sample(rep_len(seq(1, nfolds), n), n, replace = false);
  /*
  NumericVector lambdas(nfolds);
  NumericVector mse(nfolds);
  
  for (int i =1; i <= nfolds; i++){
    NumericVector x_train = x[groups != i];
    NumericVector y_train = y[groups != i];
    
    NumericVector x_test = x[groups == i];
    NumericVector y_test = y[groups == i];
    
   // opt = optim_lbfgs(f = function(x){mse_Rcpp(x, x_train, y_train, x_test, y_test)}, double x = 0.02, double opt_mse)
  }
  */
  return 0;
}
