...
library(pROC)
roc_data <- results %>%
    mutate(Group = as.factor(paste("Group", Group)),
    True_Anomaly = as.factor(paste("Group", True_Anomaly))) %>%
    select(- Distances) %>%
    pivot_wider(names_from = Group, values_from = Probability)


roc_fit <- multiclass.roc(
    response = roc_data$True_Anomaly,
    predictor = as.matrix(roc_data[, 5:14]),
    ret = TRUE)
``` 

```{r}
library(ROCR)
roc_predict <- prediction(
    predictions = as.matrix(roc_data[, 5:14]),
    labels = roc_data$True_Anomaly)
```
