...
This document's goals are two-fold:
...
tree <- move_tip(tree, "t10", tree$node.label)
...
```{r, eval= FALSE}
tree <- as.phylo(original$tree)
tree <- makeNodeLabel(tree)

tip <- sample(1:10, 2)

parent <- sample(tip:9, N:Node)
tree <- move_node(tree, tree$node.label[tip], tree$node.label[parent])
